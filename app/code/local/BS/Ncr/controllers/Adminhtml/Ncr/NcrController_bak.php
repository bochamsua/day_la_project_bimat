    ...
    public function saveAction()
    {
        if ($data = $this->getRequest()->getPost('ncr')) {
            try {
                ...
                if( $ncr->getId()){//$roleId == 3 &&, this will lead to direct input param exploit but it's fine for now

	                if($this->getRequest()->getParam('accept')){
		                $ncr->setNcrStatus(2);
		                $ncr->setRejectReason('');
		                $ncr->setApprovalId($currentUser[0]);
		                $message = 'accepted';
		                //$data['ncr_status'] = 2;
		                ...
		                //$data['ncr_status'] = 0;
		                $ncr->setNcrStatus(0);
		                $ncr->setRejectReason($data['reject_reason']);
		                $message = 'rejected';
		                $ncr->save();
