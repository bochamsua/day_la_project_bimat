        ...
        if ($data = $this->getRequest()->getPost('qr')) {
            try {
                ...
                /*if(!$qr->getId()){
                    ...
                    $data['ref_no'] = Mage::helper('bs_qr')->getNextRefNo();
	                $data['qr_status'] = 0;
                }*/

	            $message = 'saved';

	            if( $qr->getId()){//$roleId == 3 &&, this will lead to direct input param exploit but it's fine for now

		            if($this->getRequest()->getParam('accept')){
			            $qr->setQrStatus(2);
			            $qr->setRejectReason('');
			            $qr->setApprovalId($currentUser[0]);
			            $message = 'accepted';
			            //$data['ncr_status'] = 2;
			            ...
			            Mage::getSingleton('adminhtml/session')->addSuccess(
				            Mage::helper('adminhtml')->__('QR was successfully %s.', $message)
			            );
			            ...
			            //$data['ncr_status'] = 0;
			            $qr->setQrStatus(0);
			            $qr->setRejectReason($data['reject_reason']);
			            $message = 'rejected';
			            $qr->save();
			            ...
			            $qr->setData('qr_source', $ncrSourceName);
