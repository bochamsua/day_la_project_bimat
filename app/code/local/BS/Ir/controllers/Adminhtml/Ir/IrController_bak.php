...
class BS_Ir_Adminhtml_Ir_IrController extends BS_Sur_Controller_Adminhtml_Sur
{
    ...
    public function saveAction()
    {
        if ($data = $this->getRequest()->getPost('ir')) {
            try {
                $data = $this->_filterDates($data, array('report_date', 'event_date'));
                $ir = $this->_initIr();
                ...
                /*if(!$ir->getId()){
                    ...
                    $data['ref_no'] = Mage::helper('bs_misc/relation')->getNextRefNo('ir', $currentUser[2]);
	                $data['ir_status'] = 0;
                }*/
	            ...
	            if( $ir->getId()){//$roleId == 3 &&, this will lead to direct input param exploit but it's fine for now

		            if($this->getRequest()->getParam('accept')){
			            $ir->setIrStatus(2);
			            //$ir->setRejectReason('');
			            $ir->setApprovalId($currentUser[0]);
			            $message = 'accepted';
			            //$data['ncr_status'] = 2;
			            ...
			            Mage::getSingleton('adminhtml/session')->addSuccess(
				            Mage::helper('bs_ir')->__('Ir was successfully %s.', $message)
			            );
			            ...
			            //$data['ncr_status'] = 0;
			            $ir->setIrStatus(0);
			            $ir->setRejectReason($data['reject_reason']);
			            $message = 'rejected';
			            $ir->save();

			            Mage::getSingleton('adminhtml/session')->addSuccess(
				            Mage::helper('bs_ncr')->__('Ir was successfully %s.', $message)
			            );
