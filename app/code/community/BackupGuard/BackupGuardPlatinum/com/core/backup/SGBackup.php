...
class SGBackup implements SGIBackupDelegate
{
	...
	private function cleanUp()
	{
        ...
        /* S2L Solutions <info@s2lsolutions.com> Added date: 28/5/17*/
        //Disable cache
        ...
        //logout
        //$url = Mage::getModel('core/url')->getUrl('adminhtml/index/logout');
	...
	private static function getBackupFileName()
	{
	    ...
	    $type = 'full_';
	    if($request['backupType'] == 2){
	        $type = '';
            if($request['backupDatabase'] == "on"){
                $type .= 'db_';
            }
            if($request['backupFiles'] == "on"){
                $type .= 'files_'.implode('', $request['directory']).'_';
            }
        }
		...
		return $sgBackupPrefix.$type.(@date('YmdHis'));
	}
