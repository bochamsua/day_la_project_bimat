    ...
    */
	...
	protected function _setCollectionOrder($column)
	{
		...
		if ($collection) {
			...
			/* S2L Solutions <info@s2lsolutions.com> Added date: 3/3/17*/
			if($column->getId() == 'ref_no'){
    ...
    protected function _getExportHeaders()
    {
        ...
        $keys = array_keys($this->getCollection()->getFirstItem()->getData());
        $type = $this->getType();//bs_ncr/adminhtml_ncr_grid
        $type = explode("/", $type);
        $type = explode("_", $type[0]);
        $type = $type[1];

        foreach ($keys as $key) {
            if (!in_array($key,['created_at', 'updated_at', 'ac', 'is_submitted', 'accept', 'status', 'reg', 'task_code', 'sub_code','ref_id'])) {
                $row[] = Mage::helper('bs_misc/keymap')->getExportHeader($type, $key);
            }
        }
        return $row;
    }
    ...
    public function _exportIterateCollection($callback, array $args)
    {
        ...
        while ($break !== true) {
            ...
            //custom filter region/section because these columns were not included in collection columns
            $filter   = $this->getParam($this->getVarNameFilter(), null);
            $data = [];
            if (is_string($filter)) {
                $data = $this->helper('adminhtml')->prepareFilterString($filter);
            ...
            if(isset($data['region'])){
                $collection->addFieldToFilter('region', $data['region']);
            }
            if(isset($data['section'])){
                $collection->addFieldToFilter('section', $data['section']);
            }
    ...
    protected function _exportExcelItem(Varien_Object $item, Varien_Io_File $adapter, $parser = null)
    {
        ...
        foreach ($keys as $key) {
            if (!in_array($key,['created_at', 'updated_at', 'ac', 'is_submitted', 'accept', 'status', 'reg', 'task_code', 'sub_code','ref_id'])) {
                $row[] = Mage::helper('bs_misc/keymap')->getExportValue($item, $type, $key);
            }
